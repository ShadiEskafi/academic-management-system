...
export function renderAssignmentFormModal({
    ...
    <div class="modal-content" style="max-width: 460px;">
      ...
      <form id="assignment-modal-form">
        <div style="margin-bottom:0.85rem;">
          <label style="font-weight:600;display:block;margin-bottom:4px;">العنوان *</label>
          <input
            type="text"
            name="title"
        ...
        <div style="margin-bottom:0.85rem;">
          <label style="font-weight:600;display:block;margin-bottom:4px;">نوع المهمة</label>
          <select name="type">
        ...
        <div style="display:flex;gap:0.75rem;margin-bottom:0.85rem;">
          <div style="flex:1;">
            <label style="font-weight:600;display:block;margin-bottom:4px;">تاريخ الاستحقاق</label>
            <input type="date" name="dueDate" value="${initialData?.date || ''}" />
          </div>
          <div style="flex:1;">
            <label style="font-weight:600;display:block;margin-bottom:4px;">الوقت التقديري (دقيقة)</label>
            <input type="number" name="estimatedMinutes" min="0" placeholder="مثال: 60" value="${initialData?.estimated_minutes || ''}" />
          </div>
        </div>

        <div style="display:flex;gap:0.75rem;margin-bottom:0.85rem;">
          <div style="flex:1;">
            <label style="font-weight:600;display:block;margin-bottom:4px;">الأولوية</label>
          ...
          <div style="flex:1;">
            <label style="font-weight:600;display:block;margin-bottom:4px;">الحالة</label>
            <select name="status">
              <option value="pending" ${!initialData || initialData?.status === 'pending' ? 'selected' : ''}>قيد الانتظار</option>
              <option value="in_progress" ${initialData?.status === 'in_progress' ? 'selected' : ''}>قيد الحل</option>
              <option value="completed" ${initialData?.status === 'completed' ? 'selected' : ''}>مكتمل ومسلّم</option>
            </select>
          </div>
        </div>

        <p id="assignment-modal-error" style="color:#ef4444;font-size:13px;margin:0 0 0.5rem;"></p>
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!title) {
      ...
      saveBtn.textContent = isEditing ? 'حفظ التعديلات' : 'إضافة الواجب';
      return;
    }
